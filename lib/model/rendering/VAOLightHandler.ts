...
//import {GameSurfaceGLCamera, GameSurfaceGLLight} from "./GameSurfaceGL";
...
export class VAOLightHandler extends VAOHandlerInterface {

    shadowTextureResolution: Vec2 = (new Vec2()).set(1024, 1024);
    frameBuffer;
    depthTexture = null;
    type: string = "Light";
    ...
    public update(gl) {
        ...
        if (!(this.attributes.vertColor)) {
            console.log({message: `VAOLightHandler getAttribLocation no camera attribute pointer vertColor`})
        }
        if (!(this.attributes.vertNormal)) {
            console.log({message: `VAOLightHandler getAttribLocation no camera attribute pointer vertNormal`})
        }
    ...
    public setupFrameBuffer() {
        if (!this.frameBuffer) {
            console.log("GENERATING FRAME BUFFER");
            ...
            this.gl.texImage2D(this.gl.TEXTURE_2D, 0, this.gl.RGBA, this.shadowTextureResolution.x, this.shadowTextureResolution.y, 0, this.gl.RGBA, this.gl.UNSIGNED_BYTE, null);

            const renderBuffer = this.gl.createRenderbuffer();
            this.gl.bindRenderbuffer(this.gl.RENDERBUFFER, renderBuffer);
            this.gl.renderbufferStorage(this.gl.RENDERBUFFER, this.gl.DEPTH_COMPONENT16, this.shadowTextureResolution.x, this.shadowTextureResolution.y);
        ...
        this.gl.viewport(0, 0, this.shadowTextureResolution.x, this.shadowTextureResolution.y);
    ...
    renderFrameBuffer() {
        ...
        this.gl.blitFramebuffer(0, 0, this.shadowTextureResolution.x, this.shadowTextureResolution.y, 0, 0, this.shadowTextureResolution.x, this.shadowTextureResolution.y,
