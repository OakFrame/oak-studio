...
void main()
{
    ...
    fragColor = vertColor;
    /* vNormal = (mWorld * vec4(vertNormal, 0)).xyz;//(mWorld * vec4(vertNormal, 0)).xyz;
     vPosition = (mProj * mWorld * vec4(vertPosition, 0)).xyz;
     vColor = vertColor;
     gl_Position = mProj * mWorld * vec4(vertPosition, 1.0);

     vec4 vertPos4 = mProj * mWorld * vec4(vertPosition, 1.0);
     vNormal = vertNormal;//, 0.0);////vec3(mWorld * vec4(vertNormal, 0.0));
     mNormal = vec3(mWorld * vec4(vertNormal, 0.0));
