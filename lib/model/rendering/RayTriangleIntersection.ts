...
export function rayTriangle(origin: Vec3, direction: Vec3, tri: Face3): false | Vec3 { // rayTriangle(from:Vec3, direction:Vec3 triangle:Face3)
    ...
    if (Tri3_Temp.e != Math.sign(Tri3_Temp.u) || 0 == Tri3_Temp.e) return false;
    ...
    if (0 > Tri3_Temp.c * Tri3_Temp.a + Tri3_Temp.h * Tri3_Temp.i + Tri3_Temp.M * Tri3_Temp.t) return false;
    ...
    return 0 > Tri3_Temp.c * Tri3_Temp.a + Tri3_Temp.h * Tri3_Temp.i + Tri3_Temp.M * Tri3_Temp.t ? false : new Vec3().set(Tri3_Temp.f, Tri3_Temp.g, Tri3_Temp.s); //[f, g, s, vecDist(y, [f, g, s])];
}

export function sphereTriangle(sphereCenter: Vec3, sphereRadius: number, triangle: Face3): false | Vec3 {
    const triangleNormal = triangle.getnormal();
    const signedDistance = triangleNormal.dot(sphereCenter) - triangleNormal.dot(triangle.getcenter());
    const absDistance = Math.abs(signedDistance);
    if (absDistance > sphereRadius) {
        return false;
    }

    const closestPoint = triangle.closestPointToPoint(sphereCenter);
    const distSquared = closestPoint.distanceToSquared(sphereCenter);
    const radiusSquared = sphereRadius * sphereRadius;
    if (distSquared > radiusSquared) {
        return false;
    }

    if (absDistance <= 0.0001) {
        return closestPoint;
    }

    const normalDirection = signedDistance < 0 ? 1 : -1;
    const penetrationDepth = normalDirection * (sphereRadius - absDistance);
    const penetrationVector = triangleNormal.clone().mulI(penetrationDepth);
    return closestPoint.add(penetrationVector);
}
